...
def process_google_form_survey(csv_path):
    ...
    for index, row in survey_data.iterrows():
        ...
        try:
            ...
            pass # Dòng lỗi nhập chữ thay vì số thì cho qua
        ...
        if "spotify.com" in url:
            try:
                ...
                cmd = f'"{sys.executable}" spotify_process.py "{pl_id}" "{raw_csv}"'
            except:
                print("⚠️ Link Spotify không hợp lệ.")
                continue
        elif "apple.com" in url:
            cmd = f'"{sys.executable}" apple_music_process.py "{url}" "{raw_csv}"'
        elif "youtube.com" in url or "youtu.be" in url:
            cmd = f'"{sys.executable}" youtube_process.py "{url}" "{raw_csv}"'
        else:
            print(f"⚠️ Nền tảng nhạc lạ chưa hỗ trợ: {url}")
            continue
